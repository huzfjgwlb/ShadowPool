...
class Wide_ResNet_moe(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, n_expert=4):
        ...
        self.layer1 = nn.ModuleList([self._wide_layer(1, wide_basic, nStages[1], n, dropout_rate, stride=1, in_planes=self.in_planes) for _ in range(n_expert)])
        self.layer2 = nn.ModuleList([self._wide_layer(2, wide_basic, nStages[2], n, dropout_rate, stride=2, in_planes=160) for _ in range(n_expert)])
        self.layer3 = nn.ModuleList([self._wide_layer(3, wide_basic, nStages[3], n, dropout_rate, stride=2, in_planes=320) for _ in range(n_expert)])
