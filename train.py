...
from utils.utils import progress_bar, set_random_seed, get_dataset
...
if args.num_shadow is not None:
    if args.split_type == 'random':
        ...
        keep = keep.nonzero()[0]
    elif args.split_type == 'eight':
        keep = np.random.uniform(0, 1, size=(args.num_shadow, dataset_size))
        order = keep.argsort(0)
        keep = order < int(args.pkeep * args.num_shadow)
        keep = np.array(keep[args.shadow_id % 8], dtype=bool) #one
        keep = keep.nonzero()[0]
else:
    # get target dataset
...
net.cuda()
tb_writer = SummaryWriter(f'runs/{args.dataset}-{args.net}-{args.split_type}-{args.pkeep}-{args.shadow_id}')
...
#     "in_data": keep,
#     }
dir='saved_models/' + args.name + '-' + args.dataset + '-' + args.split_type + '-' + str(args.pkeep)
os.makedirs(dir, exist_ok=True)
if args.num_shadow:
    torch.save(state, f'{dir}/{args.name}_shadow_{args.shadow_id}_last.pth')
    # torch.save(state_data, f'{dir}/{args.name}_shadow_{args.shadow_id}_last_idx.pth')
else:
    torch.save(state, f'{dir}/{args.name}_target_last.pth')
    # torch.save(state_data, f'{dir}/{args.name}_target_last_idx.pth')
