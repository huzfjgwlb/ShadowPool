...
class CustomDataset(Dataset):
    def __init__(self, original_dataset, pathways):
        self.original_dataset = original_dataset
        self.dataset_size = len(original_dataset)
        self.pathways = pathways
        self.pathway_num = len(pathways)
    ...
    def __getitem__(self, index):
        ...
        additional_label = self.pathways[index % self.pathway_num]
        return img, label, additional_label

    def get_additional_labels(self):
        additional_labels = []
        for index in range(self.dataset_size):
            additional_labels.append(self.pathways[index % self.pathway_num])
...
class CustomDataset2(Dataset):
    ...
    def __getitem__(self, index):
        img, label = self.original_dataset[index]
        additional_label = self.pathways
        return img, label, additional_label

    def get_additional_labels(self):
        additional_labels = []
        for index in range(self.dataset_size):
            additional_labels.append(self.pathways)
        additional_labels = np.array(additional_labels)
        return additional_labels

def entire_combinations(arrays):
    combinations = product(*arrays)
    tmp = []
    for combination in combinations:
        tmp.append(list(combination))
    return tmp


# refer: https://github.com/OPTML-Group/Robust-MoE-CNN/utils.general_utils
...
def get_logits_moe(curr_canary, model, router=None, keep_tensor=False):
    with torch.no_grad():
        logits = model(curr_canary, router)
...
def lira_online(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels, fix_variance=False):
    ...
    for j in range(shadow_scores.shape[1]):
        if shadow_scores[shadow_in_out_labels[:, j], j, :].shape[0] == 0:
            print(j, 'error 0')
        if  shadow_scores[~shadow_in_out_labels[:, j], j, :].shape[0] == 0:
            print(j, 'error 1')
    ...
    print('online size:', in_size, out_size)
...
def cal_results(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels, logits_mul=1, logits_strategy=None):
    ...
    some_stats['fix_auc'] = round(auc, 4)
    some_stats['fix_acc'] = round(acc, 4)
    some_stats['fix_TPR@0.01FPR'] = round(low, 4)

    final_preds, true_labels = lira_offline(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels,
                                        fix_variance=True)
    fpr, tpr, auc, acc, low = cal_stats(logits_mul * final_preds, true_labels)
    some_stats['fix_off_auc'] = round(auc, 4)
    some_stats['fix_off_acc'] = round(acc, 4)
    some_stats['fix_off_TPR@0.01FPR'] = round(low, 4)

    return some_stats

def cal_result_online(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels, logits_mul=1, logits_strategy=None):
    some_stats = {}
...
def cal_result_offline(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels, logits_mul=1, logits_strategy=None):
    some_stats = {}
...
# reference: https://github.com/microsoft/Stochastic-Mixture-of-Experts
def symmetric_KL_loss(p, q):
    """ symmetric KL-divergence 1/2*(KL(p||q)+KL(q||p)) """
    p, q = p.float(), q.float()
    dict_size = q.size(-1)
    p = p.view(-1, dict_size)
    q = q.view(-1, dict_size)
    loss = (p - q) * (torch.log(p+1e-10) - torch.log(q+1e-10)) # except: zero
    return 0.5 * loss.sum()


# reference: Protecting DNNs from Theft using an Ensemble of Diverse Models
def cos_diversity_loss(feature_maps):
    # output_list: n x [batch_size, class_num]
    batch_size = feature_maps[0].size(0)
    output = torch.stack([F.normalize(f.view(f.size(0), -1)) for f in feature_maps], dim=0)  # [n, batch_size, class_num]
    # [n, n, batch_size]
    cosine_sim_matrix = F.cosine_similarity(output.unsqueeze(1), output.unsqueeze(0), dim=-1)
    # ensure a <= b，包括对角线
    upper_triangle = torch.triu(cosine_sim_matrix, diagonal=0)
    exp_sum = torch.sum(torch.exp(upper_triangle), dim=(0, 1))  # sum over pairs (a, b) and over the batch
    loss = torch.log(exp_sum / batch_size)
    
    return loss.sum()


def enc_diversity_loss(feature_maps):
    # output_list: n x [batch_size, class_num]
    batch_size = feature_maps[0].size(0)
    output = torch.stack([f.view(f.size(0), -1) for f in feature_maps], dim=0)  # [n, batch_size, class_num]

    output_a = output.unsqueeze(1)  # [n, 1, batch_size, class_num]
    output_b = output.unsqueeze(0)  # [1, n, batch_size, class_num]
    
    euclidean_dist_matrix = torch.norm(output_a - output_b, dim=-1, p=2)  # [n, n, batch_size]
    upper_triangle = torch.triu(euclidean_dist_matrix, diagonal=0)
    exp_sum = torch.sum(torch.exp(upper_triangle), dim=(0, 1))
    loss = torch.log(exp_sum / batch_size)
    
    return loss.sum()


def simple_cos_diversity_loss(feature_maps):
    """
    使用矩阵运算计算不同特征张量之间的距离损失
    """
    loss = 0.0
    num_features = len(feature_maps)

    # 将每个特征张量展平成二维张量 (batch_size, channels*height*width) 并归一化
    flattened_features = [F.normalize(f.view(f.size(0), -1), dim=1) for f in feature_maps]

    # 计算每对特征张量之间的余弦相似度（或者使用欧氏距离）
    for i in range(num_features):
        for j in range(i + 1, num_features):
            f_i = flattened_features[i]
            f_j = flattened_features[j]
            # 计算余弦距离
            cosine_similarity = torch.matmul(f_i, f_j.T)  # (batch_size, batch_size)
            mean_distance = torch.mean(cosine_similarity)  # 批次中的平均距离
            loss += mean_distance

    return -loss  # 距离越远越好，取负

def simple_enc_diversity_loss(feature_maps):
    """
    使用矩阵运算计算不同特征张量之间的欧式距离损失
    """
    loss = 0.0
    num_features = len(feature_maps)

    # 将每个特征张量展平成二维张量 (batch_size, channels*height*width)
    flattened_features = [f.view(f.size(0), -1) for f in feature_maps]
    ...
    for i in range(num_features):
        for j in range(i + 1, num_features):
            ...
            f_j = flattened_features[j]
            # 计算欧式距离
            distance = torch.sqrt(torch.sum((f_i - f_j) ** 2, dim=1))  # (batch_size,)
            mean_distance = torch.mean(distance)  # 批次中的平均距离
            loss += mean_distance

    return loss  # 欧式距离越远越好


def orth_diversity_loss(feature_maps):
    loss = 0.0
    num_features = len(feature_maps)
    ...
    for i in range(num_features):
        for j in range(i + 1, num_features):
            f_i = flattened_features[i]
            f_j = flattened_features[j]
            dot_product = torch.matmul(f_i, f_j.T)  # (batch_size, batch_size)
            mean_dot_product = torch.mean(torch.abs(dot_product)) 
            loss += mean_dot_product

    return loss


def orth_diversity_loss_v2(feature_maps):

    flattened_features = torch.stack([F.normalize(f.view(f.size(0), -1), dim=1) for f in feature_maps], dim=0)
    dot_products = torch.matmul(flattened_features, flattened_features.permute(0, 2, 1))
    mean_dot_products = torch.mean(torch.abs(dot_products), dim=(1, 2))

    loss = torch.sum(mean_dot_products) - torch.sum(torch.diagonal(mean_dot_products)) # 去掉自身与自身的计算

    return loss


diversity_loss_map = {
    "cos": cos_diversity_loss,
    "enc": enc_diversity_loss,
    "simple_enc": simple_enc_diversity_loss,
    "simple_cos": cos_diversity_loss,
    "orth": orth_diversity_loss,
    "orth_v2": orth_diversity_loss_v2
}
